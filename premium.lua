...
ImageButton.Image = "http://www.roblox.com/asset/?id=116028320260198"
...
local win = library:Window(" PREMIUM",[[VIP]],"12235359506",Enum.KeyCode.RightControl)
local Main = win:Tab("trang chủ",[[6026568198]])
local Stats = win:Tab("Stats",[[7040410130]])
local Combat = win:Tab("lụm bounty",[[7251993295]])
local Teleport = win:Tab("dịch chuyển",[[6035190846]])
local Dungeon = win:Tab("raid",[[7044284832]])
local Shop = win:Tab("cửa hàng",[[6031265976]])
local DevilFruit = win:Tab("Devil Fruit",[[7044233235]])
local Misc = win:Tab("cài đặt",[[6034509993]])
	...
	while wait() do
		pcall(function()
			...
			elseif string.len(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CakePrinceSpawner")) == 86 then
				...
				MobKilled:Set("Boss đã Spawning 🟢")
			end
		end)
	end
end)
	...
	pcall(function()
		while wait() do
			if game.Workspace._WorldOrigin.Locations:FindFirstChild('Mirage Island') then
				Mirragecheck:Set('  Mirage Spawning rồi 🟢')
			else
				Mirragecheck:Set('  Mirage có con cak ❌')
			end
		end
	end)
end)
...
Main:Button("Teleport ngôi đền thời gian",function()
		...
		while wait() do
			if _G.AutoObservation then
				if game:GetService("Players").LocalPlayer.VisionRadius.Value >= 3000 then
					...
					wait(1)
					Notification:Notify(
						{Title = "", Description = "You Have Max haki quan sát"},
...
Misc:Textbox("join ID","",true,function(value)
	JobiJoin = Value
end)
